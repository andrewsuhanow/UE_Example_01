...
// #include "TEST/MyTacticMove_InventorDEBUG.h"
#include "MyTacticMove_InventorDEBUG.h"
...
void AMyTacticMove_InventorDEBUG::SelectUnit_1()
{
	SelectUnit_Performance(SelectUnitName_1);
}
void AMyTacticMove_InventorDEBUG::SelectUnit_2()
{
	SelectUnit_Performance(SelectUnitName_2);
}
void AMyTacticMove_InventorDEBUG::SelectUnit_3()
{
	SelectUnit_Performance(SelectUnitName_3);
}
void AMyTacticMove_InventorDEBUG::SelectUnit_Performance(FName _SelectUnitName)
{
...
void AMyTacticMove_InventorDEBUG::InitTestInvertorItems()
{
...
void AMyTacticMove_InventorDEBUG::OpenEquipPanel()
{
...
void AMyTacticMove_InventorDEBUG::AddItemToEquipPanel()
{
...
void AMyTacticMove_InventorDEBUG::SetEquipPanelSet()
{
	if (SelectTestUnit)
	{
...
AMyTacticMove_InventorDEBUG::AMyTacticMove_InventorDEBUG()
{

}


void AMyTacticMove_InventorDEBUG::BeginPlay()
{
	Super::BeginPlay();

}

void AMyTacticMove_InventorDEBUG::Tick(float DeltaTime)
{
...
void AMyTacticMove_InventorDEBUG::PrepareMarcer(FVector _EndPoint, AActor* _Target)
{
	DrawDebugLine(
		GetWorld(),
		SelectTestUnit->GetActorLocation(),	// ** Start
...
void AMyTacticMove_InventorDEBUG::SetPrepare__________TEST()
{
	if (SelectTestUnit)
	{
		...
		for (int32 i = 0; i < allUnits.Num(); i++)
		{
			...
			if (unit->GameName == UnitForAttackName)
			{
				...
				// ** shift on side
				switch (StandPositionOnTarget)
				{
				case ETacticalAttack_Side::Front:
					targetSide = 1;
					targetShift = 0;
					break;

				case ETacticalAttack_Side::Front_Left:
					targetSide = 1;
					targetShift = 50;
					break;
				case ETacticalAttack_Side::Front_Right:
					targetSide = 1;
					targetShift = -50;
					break;
				
				case ETacticalAttack_Side::Back:
					targetSide = -1;
					targetShift = 1;
					break;

				case ETacticalAttack_Side::Back_Left:
					targetSide = -1;
					targetShift = 50;
					break;

				case ETacticalAttack_Side::Back_Right:
					targetSide = -1;
					targetShift = -50;
					break;
				}
				...
				// ** TEST Position
				switch (TacticalAttackTEST)
				{

					// ** Too close  from RND side
				case ETacticalAttackTEST::TargetTooFar:
				{
					AWeaponWorldItem* WpnItm = SelectTestUnit->WeaponComponent->GetCurrWeaponItem();
					if (WpnItm)
					{
						UWeaponDT* weaponDT = WpnItm->ItemDT.WeaponDT->GetDefaultObject<UWeaponDT>();
						dist = weaponDT->MaxDist * 2.0f;
					}

					FVector newSelfPont = UnitTarget->GetActorLocation() +
						FRotator(0.f, targetShift, 0.f).RotateVector(UnitTarget->GetActorForwardVector() * targetSide * dist);

					SelectTestUnit->SetActorLocation(newSelfPont);

					FRotator AngleToTarget = UKismetMathLibrary::FindLookAtRotation(SelectTestUnit->GetActorLocation(), UnitTarget->GetActorLocation());
					SelectTestUnit->SetActorRotation(AngleToTarget);

					break;
				}
					// ** Too close  from RND side
				case ETacticalAttackTEST::TargetTooClose:
				{
					...
					{
						UWeaponDT* weaponDT = WpnItm->ItemDT.WeaponDT->GetDefaultObject<UWeaponDT>();
						dist = weaponDT->MinDist - weaponDT->MinDist / 10.f;
					}
					...
					// ** too close or after optimal
				case ETacticalAttackTEST::BeforeAttackCorrectDist:
				{
					AWeaponWorldItem* WpnItm = SelectTestUnit->WeaponComponent->GetCurrWeaponItem();
					if (WpnItm)
					{
						...
						//if (weaponDT->OptimalDist < FVector::Distance(SelectTestUnit->GetActorLocation(), UnitTarget->GetActorLocation()))
						if (RND == 0)
						{
							dist = weaponDT->OptimalDist + weaponDT->OptimalDist / 2;
						}
						else
						{
							dist = weaponDT->MinDist - weaponDT->MinDist / 2;
						}
					}
...
void AMyTacticMove_InventorDEBUG::Attack______________SET()
{
	Attack_Performance(UnitForAttackName, false);
}

void AMyTacticMove_InventorDEBUG::Attack______________ADD()
{
	Attack_Performance(UnitForAttackName, true);
}


void AMyTacticMove_InventorDEBUG::Attack_Performance(FName _UnitForAttackName, bool _bAddMoreOneTask)
{
